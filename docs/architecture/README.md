...
| django-csp Report-Only mode (not enforced) | **Explicitly documented** in `docs/security/csp/CSP_NOTES.md`: the policy is being run in Report-Only mode deliberately, to observe real traffic and close known gaps (e.g. a since-removed flag-image CDN reference) before enforcing. |
