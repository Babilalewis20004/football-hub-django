...
- [security/csp/CSP_NOTES.md](security/csp/CSP_NOTES.md) — CSP Report-Only rollout plan and directive rationale
- [security/csp/CSP_BROWSER_TESTING.md](security/csp/CSP_BROWSER_TESTING.md) — CSP browser verification walkthrough
...
| 11 | `docs/security/csp/CSP_NOTES.md` references `blog/services/homepage.py:22-23` hardcoding flag images from `https://flagcdn.com` and a `templates/components/match_block.html` template — **neither exists in the current codebase** (confirmed by direct file read and project-wide grep). This project document is stale relative to the current code; this documentation package reflects the actual current state (no flag images, no match-block feature), not that document's snapshot. | `docs/security/csp/CSP_NOTES.md` vs. current `blog/services/homepage.py` |
