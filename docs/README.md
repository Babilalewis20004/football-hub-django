...
- [architecture/deployment-architecture.md](architecture/deployment-architecture.md) — implied production topology, environment variables, dev-vs-prod differences, and what's explicitly absent (Docker, CI/CD)
...
| # | Finding | Where |
|---|---|---|
| 1 | `CustomUser.role` and the Django Group/Permission system (created by `setup_roles`) are two separate mechanisms with no code path that assigns a user to their matching Group automatically. `@permission_required` checks would fail for any user whose Group membership wasn't set manually/out-of-band. | `blog/management/commands/setup_roles.py`, `users/models.py`, confirmed via grep — only test files call `.groups.add(...)` |
| 2 | The "Contributor" Django Group has no corresponding `CustomUser.role` value (`role` choices are only `admin`/`editor`/`author`/`reader`) — unclear how any account is ever actually a Contributor. | `blog/management/commands/setup_roles.py` vs. `users/models.py` |
...
| 10 | No `Procfile`, deployment script, Dockerfile, or CI/CD configuration exists anywhere in the repository, despite both `gunicorn` and `daphne` being pinned dependencies — the actual production start command is not documented in-repo. Media files (`/media/...`) are only served by Django when `DEBUG=True`; nothing in the codebase serves them in production. | Confirmed by filesystem search; also flagged independently in `SECURITY_AUDIT_2026-08-10.md` §8 |
